...
1. Pour un projet neuf, Codex fait choisir explicitement le modèle Claude, puis inspecte le projet et fait valider une politique de revue.
...
- Pendant une revue, une progression JSONL expurgée indique la phase, la durée, les tours et les outils de lecture, avec un battement toutes les 15 secondes.
...
Lors de la première configuration, le skill recommande `claude-sonnet-5` mais demande de choisir entre Sonnet 5, Opus 4.8, Fable 5, Haiku 4.5 ou un autre identifiant exact. Il réutilise ensuite ce choix sans le redemander. Les autres valeurs par défaut sont l'effort `high`, le français, un timeout de 20 minutes et au plus 40 tours. Le snapshot est limité à 20 000 fichiers et 250 Mio ; au-delà, le runner s'arrête et demande un cadrage explicite.
...
Codex demandera d'abord le modèle Claude à enregistrer, puis inspectera le projet et proposera en une seule fois : langue, modèle choisi, effort, sources de vérité, exclusions, seuils et jalons. **Aucun fichier ne sera créé avant validation.** Après accord, le skill ajoutera :
...
Pendant l'exécution, Codex relaie la progression émise par le runner. `stdout` reste réservé au résultat JSON final et aucun journal d'échec n'est conservé sur disque. Après un arrêt brutal non interceptable, le dernier événement visible indique la dernière phase atteinte, sans pouvoir expliquer un `SIGKILL` lui-même.
