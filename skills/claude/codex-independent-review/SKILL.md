...
description: Use Codex as a separate, read-only reviewer for code, architecture, security, design, or content, with filtered snapshots, immutable audit reports, adjudication by the main agent, resolution records, user-confirmed counter-reviews, and project-level disabling that preserves prior audits. Use when the user asks for a Codex, external, or second-model review, when configuring or disabling persistent review milestones for a project, or when a CLAUDE.md pointer says a configured independent-review milestone has been reached.
---

# Revue indépendante Codex

Faire intervenir Codex uniquement comme contrôleur indépendant. Rester responsable du cadrage, de l'arbitrage des constats et des corrections.

Résoudre `<skill-dir>` comme le dossier absolu contenant ce `SKILL.md`. Exécuter toujours le runner avec `python3 <skill-dir>/scripts/codex_review.py`, quel que soit le répertoire courant du projet. Sous Windows, `py -3` remplace `python3`.
...
- Si `.claude/codex-review.json` n'existe pas, suivre **Initialiser le projet**.
...
1. Lire [references/configuration.md](references/configuration.md), puis exécuter immédiatement `python3 <skill-dir>/scripts/codex_review.py discover-models` avant toute question de configuration. Demander ensuite de manière bloquante le modèle Codex à enregistrer. Utiliser le sélecteur natif de l'environnement avec les identifiants exacts renvoyés et toujours permettre une saisie libre. Si `source` vaut `embedded_catalog`, signaler brièvement que le cache local de Codex n'était pas lisible et que le catalogue versionné est utilisé. Si le sélecteur ne peut pas présenter toute la liste, proposer d'abord `gpt-5.6-terra` et `gpt-5.6-sol` lorsqu'ils figurent dans le résultat, compléter avec les premiers modèles renvoyés, puis proposer « autres modèles disponibles » et la saisie libre. Recommander `gpt-5.6-terra` lorsqu'il figure dans le résultat, mais ne jamais choisir un modèle sans réponse.
...
5. Utiliser par défaut `high`, `fr`, 20 minutes, `docs/reviews`, 20 000 fichiers et 262 144 000 octets.
...
7. Écrire la proposition JSON dans un fichier temporaire hors du projet, puis exécuter :
   ...
   `python3 <skill-dir>/scripts/codex_review.py install-policy --project <racine> --config <proposition.json>`
...
Pour un ancien brouillon `schema_version: 0`, utiliser `python3 <skill-dir>/scripts/codex_review.py migrate-config --input <ancien> --output <nouveau>`, faire valider les différences, puis installer le nouveau fichier. Ne jamais migrer implicitement une politique de projet.

Le programme ne conserve dans le projet que la politique et le bloc délimité dans `CLAUDE.md`. En cas de bloc incomplet ou de conflit d'instructions, arrêter et demander un arbitrage.

## Désactiver dans le projet

1. Expliquer que la désactivation retire uniquement `.claude/codex-review.json` et le bloc délimité `codex-independent-review` dans le `CLAUDE.md` racine. Préciser que tous les rapports, résolutions et dossiers de preuves restent en place et que `/docs/reviews/` reste dans `.gitignore`.
...
La commande est idempotente. Elle refuse un bloc `CLAUDE.md` incomplet ou dupliqué avant de retirer la politique et restaure l'état initial si l'une des opérations échoue. Une réactivation ultérieure suit **Initialiser le projet** et ne remplace pas les audits conservés.

## Préparer une revue

1. Exécuter `python3 <skill-dir>/scripts/codex_review.py doctor` puis le même runner avec `validate-config --project <racine>`.
2. Formuler une mission neutre : objectif, résultat attendu, périmètre, angles de revue, critères, exclusions et preuves de tests déjà obtenues. Ne pas révéler ses propres conclusions ni un défaut attendu lors d'une revue générale.
...
```text
python3 <skill-dir>/scripts/codex_review.py review \
  --project <racine> \
...
Utiliser `--kind counter --confirm-counter-review` pour une contre-revue autonome confirmée, ou `--kind counter --sdp-authorized` pour une contre-revue préautorisée par l'état v4. Passer toujours `--progress` lorsque le skill lance une revue. Le runner émet alors sur `stderr` une progression JSONL expurgée avec les phases, la durée, les tours observés et un battement toutes les 15 secondes ; ne pas interpréter ces lignes comme le rapport. Il réserve `stdout` au résultat JSON final.

Le runner travaille dans un snapshot temporaire, lance `codex exec` sous bac à sable `read-only` sans configuration utilisateur, sans règles d'execpolicy, sans session persistée et sans accès web, et ne conserve aucun snapshot, log, cache ou artefact d'échec. Après une interruption contrôlée, expliquer l'erreur à partir de la dernière phase visible. Un arrêt non interceptable comme `SIGKILL` ne peut révéler que le dernier événement déjà émis.

**Une revue n'est bornée que par `timeout_minutes`.** Le CLI Codex n'expose aucun plafond de tours ; ne pas promettre une limite en tours et ne pas traiter `turns_observed` comme une garantie.
...
5. Exécuter soi-même les vérifications pertinentes ; Codex n'exécute aucune commande. Préférer les options sans cache ni artefact, inventorier l'état avant le contrôle et supprimer seulement les fichiers jetables dont la création par cette passe est certaine. Ne jamais supprimer un cache ou un build préexistant.
6. Créer un fichier de résolution distinct seulement s'il existe des constats à arbitrer ou traiter. Référencer le rapport, les décisions, les corrections et les contrôles. Ne jamais réécrire le rapport Codex.
...
- Le runner charge automatiquement [references/reviewer-mandate.md](references/reviewer-mandate.md) et le place en tête du prompt ; ne pas recopier ce mandat dans la mission.
- Utiliser `scripts/codex_review.py` pour toute invocation Codex afin de préserver les verrous et l'hygiène des projets.
