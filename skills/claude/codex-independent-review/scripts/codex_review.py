#!/usr/bin/env python3
"""Run isolated, read-only Codex reviews with durable audit evidence."""
...
SCHEMA_VERSION = 1
CONFIG_RELATIVE = Path(".claude/codex-review.json")
SDP_STATE_RELATIVE = Path(".sdp/state.json")
REVIEW_IGNORE_RULE = "/docs/reviews/"
POINTER_FILENAME = "CLAUDE.md"
POINTER_START = "<!-- codex-independent-review:start -->"
POINTER_END = "<!-- codex-independent-review:end -->"
...
MODEL_CATALOG_VERSION = "2026-08-08"
MODEL_CATALOG = (
    {
        "id": "gpt-5.6-terra",
    ...
    },
    {
        ...
        "profile": "Génération précédente pour le code complexe et la recherche.",
        "recommended": False,
    },
    {
        "id": "gpt-5.4",
    ...
    },
    {
        ...
        "profile": "Petit modèle rapide, pour une revue de faible enjeu.",
        "recommended": False,
    },
)
MODEL_CATALOG_BY_ID = {item["id"]: item for item in MODEL_CATALOG}
MODELS_CACHE_RELATIVE = Path("models_cache.json")
MODELS_CACHE_MAX_BYTES = 2 * 1024 * 1024
...
FORBIDDEN_HELP_FLAGS = {
    "--dangerously-bypass-approvals-and-sandbox",
    "--dangerously-bypass-hook-trust",
}
WINDOWS_CODEX_HINTS = (
    Path("AppData/Local/OpenAI/Codex/bin"),
)
POSIX_CODEX_HINTS = (
    Path(".local/bin"),
    Path(".codex/bin"),
...
def validate_policy(data: dict[str, Any]) -> Policy:
    _expect_keys(
        data,
        {"schema_version", "project", "codex", "reports", "context_files", "snapshot", "milestones"},
        "configuration",
    )
    ...
    for value, context in ((project, "project"), (codex, "codex"), (reports, "reports"), (snapshot, "snapshot")):
        if not isinstance(value, dict):
            raise ReviewError(f"{context} doit être un objet")

    _expect_keys(project, {"name", "language"}, "project")
    _expect_keys(codex, {"model", "effort", "timeout_minutes"}, "codex")
    _expect_keys(reports, {"directory"}, "reports")
    _expect_keys(snapshot, {"max_files", "max_bytes", "extra_excludes"}, "snapshot")

    effort = _string(codex["effort"], "codex.effort")
    if effort not in ALLOWED_EFFORTS:
        raise ReviewError(f"codex.effort doit être dans : {', '.join(sorted(ALLOWED_EFFORTS))}")
    model = _string(codex["model"], "codex.model")
    if model.lower() in MOVING_MODEL_ALIASES:
        raise ReviewError("codex.model doit être un identifiant exact, pas un alias mouvant")
    ...
    if report_directory == ".claude" or report_directory.startswith(".claude/"):
        raise ReviewError("reports.directory ne peut pas se trouver dans .claude")
    ...
    return Policy(
        ...
        model=model,
        effort=effort,
        timeout_minutes=_integer(codex["timeout_minutes"], "codex.timeout_minutes", 1, 120),
...
def migrate_policy(data: dict[str, Any]) -> dict[str, Any]:
    ...
    codex = migrated.get("codex")
    snapshot = migrated.get("snapshot")
    if not isinstance(codex, dict) or not isinstance(snapshot, dict):
        raise ReviewError("Le brouillon version 0 doit contenir les objets codex et snapshot")
    codex.setdefault("timeout_minutes", 20)
    codex.pop("max_turns", None)
...
def _pointer_block(language: str) -> str:
    if language.lower().startswith("fr"):
        body = (
            "## Revue indépendante Codex\n\n"
            "Lorsque `.claude/codex-review.json` existe, utiliser `codex-independent-review` aux jalons applicables "
        ...
        body = (
            ...
            "When `.claude/codex-review.json` exists, use `codex-independent-review` at applicable milestones "
...
def render_pointer(existing: str | None, language: str) -> str:
    block = _pointer_block(language)
    if existing is None or not existing.strip():
        return block + "\n"
    has_start = POINTER_START in existing
    has_end = POINTER_END in existing
    if has_start != has_end:
        raise ReviewError(f"Bloc codex-independent-review incomplet dans {POINTER_FILENAME} ; arbitrage manuel requis")
    if has_start:
        if existing.count(POINTER_START) != 1 or existing.count(POINTER_END) != 1:
            raise ReviewError(
                f"Plusieurs blocs codex-independent-review dans {POINTER_FILENAME} ; arbitrage manuel requis"
            )
        start = existing.index(POINTER_START)
        end = existing.index(POINTER_END, start) + len(POINTER_END)
        return existing[:start] + block + existing[end:]
    separator = "" if existing.endswith("\n\n") else "\n" if existing.endswith("\n") else "\n\n"
    return existing + separator + block + "\n"


def render_pointer_without_block(existing: str | None) -> str | None:
...
def _exclusive_state_lock(state_path: Path) -> Iterable[None]:
    lock_root = Path(tempfile.gettempdir()) / "codex-independent-review-locks"
...
def _evaluate_sdp_authorization(
    ...
    *,
    ...
    if reviews.get("mode") != "codex":
        raise ReviewError("L'état v4 n'active pas le reviewer Codex pour cette version")
...
def install_policy(project_root: Path, proposal_path: Path, replace: bool) -> dict[str, Any]:
    ...
    pointer_path = project_root / POINTER_FILENAME
    gitignore_path = project_root / ".gitignore"
    _ensure_output_within(project_root, config_path, "Politique")
    _ensure_output_within(project_root, pointer_path, f"Pointeur {POINTER_FILENAME}")
    ...
    existing_pointer = old_pointer.decode("utf-8") if old_pointer is not None else None
    try:
    ...
    rendered_pointer = render_pointer(existing_pointer, policy.language).encode("utf-8")
    ...
    try:
        ...
        _atomic_write(pointer_path, rendered_pointer, replace=True)
        ...
        try:
            ...
            if old_pointer is None:
                ...
                _atomic_write(pointer_path, old_pointer, replace=True)
            ...
            if claude_directory.exists() and not any(claude_directory.iterdir()):
...
def disable_policy(project_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ReviewError(f"{POINTER_FILENAME} doit être encodé en UTF-8 pour retirer le pointeur géré") from exc
    rendered_pointer = render_pointer_without_block(existing_pointer)
    rendered_pointer_bytes = rendered_pointer.encode("utf-8") if rendered_pointer is not None else None

    removed: list[str] = []
    try:
        if rendered_pointer_bytes is not None and rendered_pointer_bytes != old_pointer:
            _atomic_write(pointer_path, rendered_pointer_bytes, replace=True)
            removed.append("claude_pointer")
...
def _select_candidates(
    ...
    default_patterns = [*report_directories, ".claude"] if git_mode else [*DEFAULT_EXCLUDED_DIRS, *report_directories]
...
def compose_prompt(
    ...
    Codex has no equivalent of an appended system prompt, so the reviewer mandate
    travels in-band, ahead of the mission, and is fenced off from it.
    """
    ...
    briefing = textwrap.dedent(
        ...
        Mission exacte de l'utilisateur ou de l'agent principal :
        ---
        {mission.rstrip()}
        ---
        ...
        Preuves brutes de tests déjà exécutés par l'agent principal : {', '.join(test_paths) if test_paths else 'aucune'}.

        Applique le mandat de contrôleur indépendant ci-dessus. Inspecte toi-même les fichiers utiles. Retourne
        uniquement le rapport Markdown ; n'offre pas d'appliquer les corrections et ne demande aucune permission
        supplémentaire.
        """
    ).strip()
...
def _normalize_cached_models(items: Any) -> list[dict[str, Any]]:
    if not isinstance(items, list):
        raise ReviewError("Le cache de modèles Codex ne contient pas de liste de modèles")
        ...
        models.append(
            {
                ...
                "profile": (
                    ...
                    if catalogued
                        ...
                        else "Modèle découvert dans le cache Codex ; vérifier son profil avant de le retenir."
                    )
                ),
                "recommended": bool(catalogued and catalogued["recommended"]),
                **({"supported_efforts": efforts} if efforts else {}),
            }
        )
    if not models:
        raise ReviewError("Le cache de modèles Codex ne contient aucun identifiant exploitable")
    return models


def discover_models(*, allow_cache: bool = True) -> dict[str, Any]:
    """Discover exact model IDs from the local Codex cache, with a versioned fallback.

    Codex authenticates through the local CLI, not through an API key this runner
    can use, so discovery reads the catalogue Codex itself maintains instead of
    calling a remote endpoint. No network request is ever made.
    """
    warnings: list[str] = []
    if allow_cache:
        cache_path = codex_home() / MODELS_CACHE_RELATIVE
        try:
            size = cache_path.stat().st_size
            if size > MODELS_CACHE_MAX_BYTES:
                raise ReviewError("Le cache de modèles Codex dépasse la limite de 2 Mio")
            payload = load_json(cache_path)
            models = _normalize_cached_models(payload.get("models"))
            return {
                "status": "discovered",
                "source": "codex_models_cache",
                "catalog_version": MODEL_CATALOG_VERSION,
                "cache_path": str(cache_path),
                "cache_fetched_at": payload.get("fetched_at"),
                "models": models,
                "warnings": warnings,
            }
        except (ReviewError, OSError, ValueError) as exc:
            warnings.append(
                f"Cache de modèles Codex indisponible ({type(exc).__name__}) ; catalogue embarqué utilisé."
            )
    else:
        warnings.append("Lecture du cache désactivée ; catalogue embarqué utilisé.")
    return {
        "status": "fallback",
...
def _candidate_codex_paths() -> list[Path]:
    ...
    if os.name == "nt":
        for hint in WINDOWS_CODEX_HINTS:
            base = home / hint
            if base.is_dir():
                candidates.extend(sorted(base.glob("*/codex.exe")))
                candidates.append(base / "codex.exe")
    for hint in POSIX_CODEX_HINTS:
        candidates.append(home / hint / ("codex.exe" if os.name == "nt" else "codex"))
    return candidates


def resolve_codex(codex_command: str) -> str:
    resolved = shutil.which(codex_command)
    if resolved:
        return str(Path(resolved).resolve())
    if Path(codex_command).is_file():
        return str(Path(codex_command).resolve())
    for candidate in _candidate_codex_paths():
        if candidate.is_file():
            return str(candidate.resolve())
    raise ReviewError(
        "Codex CLI est introuvable dans PATH ni dans ses emplacements d'installation usuels. "
        "Installer Codex puis exécuter `codex login`, ou passer --codex <chemin>."
    )


def doctor(codex_command: str) -> dict[str, str]:
    ...
    if missing:
        raise ReviewError("Codex CLI ne prend pas en charge les verrous requis : " + ", ".join(missing))
    if "read-only" not in help_text:
        raise ReviewError("Codex CLI n'annonce pas le bac à sable read-only requis ; mettre à jour Codex")
    return {
        "codex": executable,
        "version": version,
        "auth": auth or "statut de connexion indisponible",
    }
...
def _read_process_stream(
    ...
    try:
        ...
        # The stream was closed while this reader was still blocked on it, which
        # is the normal outcome of a timeout or an interruption. Staying silent
        # keeps the progress channel on stderr machine-readable.
        pass
    finally:
...
def _stream_activity(event: dict[str, Any]) -> tuple[bool, list[str]]:
    """Best-effort progress extraction from the Codex JSONL event stream.

    The event schema of an alpha CLI is not a stable contract, so this only ever
    feeds the progress channel. Nothing about the review result depends on it.
    """
    event_type = _event_type(event)
    if not event_type:
        return False, []
    lowered = event_type.lower()
    is_turn = any(marker in lowered for marker in ("agent_message", "agent_turn", "task_started", "turn_started"))
    tools: list[str] = []
    if any(marker in lowered for marker in ("command", "tool", "exec", "patch", "search", "read")):
    ...
    with tempfile.TemporaryDirectory(prefix="codex-review-output-") as output_directory:
        ...
        try:
            ...
            if process.stdin is None or process.stdout is None or process.stderr is None:
                raise ReviewError("Impossible d'ouvrir les flux de Codex")
            try:
            ...
            unparsed_events = 0
            reporter.emit("codex", "started", model=policy.model, effort=policy.effort)

            while len(ended) < 2 or process.poll() is None:
                now = time.monotonic()
                if now >= deadline:
                    raise ReviewError(
                        f"La revue Codex a dépassé {policy.timeout_minutes} minutes ; aucun artefact créé"
                    )
                ...
                try:
                    ...
                    if now >= next_heartbeat:
                        ...
                        reporter.emit("codex", "heartbeat", **heartbeat)
                ...
                if assistant_activity:
                    turns += 1
                    reporter.emit("codex", "turn", turn=turns)
                for tool in tools:
                    last_tool = tool
                    reporter.emit("codex", "tool", turn=turns, tool=tool)

            return_code = process.wait(timeout=5)
            if return_code != 0:
                raise ReviewError(f"Codex a échoué ({return_code}) : {stderr_tail.strip() or 'aucun détail'}")
            try:
                response = last_message_path.read_text(encoding="utf-8")
            except (OSError, UnicodeError) as exc:
                raise ReviewError(f"Codex n'a produit aucun rapport lisible : {exc}") from exc
            if not response.strip():
                raise ReviewError("Codex n'a produit aucun rapport Markdown exploitable")
            usage = {
            ...
            raise ReviewError(f"Impossible de lancer Codex : {exc}") from exc
        finally:
...
def render_report(
    ...
    fields: list[tuple[str, Any]] = [
        ...
        ("codex_model", policy.model),
        ("codex_effort", policy.effort),
    ...
    title = "Revue indépendante Codex" if french else "Independent Codex review"
    mission_heading = "Mission exacte transmise à Codex" if french else "Exact mission sent to Codex"
    response_heading = "Rapport original de Codex" if french else "Original Codex report"
...
def _unique_report_target(report_root: Path, base_slug: str) -> tuple[Path, Path, str]:
    ...
    base = f"{stamp}-{_slug(base_slug)}-codex"
...
def persist_audit(
    ...
    with tempfile.TemporaryDirectory(prefix="codex-review-audit-") as staging_name:
...
def command_review(arguments: argparse.Namespace, skill_root: Path) -> dict[str, Any]:
    ...
    mandate_path = skill_root / "references/reviewer-mandate.md"
    try:
        mandate = mandate_path.read_text(encoding="utf-8")
    except (OSError, UnicodeError) as exc:
        raise ReviewError(f"Mandat de contrôleur illisible : {mandate_path}: {exc}") from exc
    ...
    with sdp_review_session(
        ...
        response, usage, codex_version = invoke_codex(
            arguments.codex,
            snapshot,
            policy,
            prompt,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor_parser = subparsers.add_parser("doctor", help="Vérifier Codex et ses verrous")
    ...
    review_parser = subparsers.add_parser("review", help="Créer une revue Codex et ses preuves d'audit")
...
def _force_utf8_streams() -> None:
    """Emit UTF-8 whatever the console encoding is.

    Results and errors are JSON with `ensure_ascii=False` and French text; on a
    Windows console defaulting to a legacy code page they would otherwise be
    mangled or raise before the caller could parse them.
    """
    ...
    try:
        with _translated_interrupts():
            if arguments.command == "doctor":
                result: Any = {"status": "ready", **doctor(arguments.codex)}
            elif arguments.command == "discover-models":
                result = discover_models(allow_cache=not arguments.offline)
                ...
                result = {
                    ...
                    "pointer": str(project_root / POINTER_FILENAME),
                    **installation,
                }
