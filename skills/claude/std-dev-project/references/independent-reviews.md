...
Rester responsable de la mission, de l'arbitrage, des corrections et des tests.
Le reviewer reste en lecture seule. Ne jamais confier l'arbitrage d'un constat
au modèle qui l'a produit.

## Choisir le parcours

1. Si le skill `codex-independent-review` est disponible **et**
   `.claude/codex-review.json` existe, choisir `reviews.mode: codex` et suivre
...
2. Sinon choisir `external-prompt`. Ne jamais initialiser silencieusement une
   politique Codex : produire le prompt portable ci-dessous.
...
Le runner de `codex-independent-review` revalide chacune de ces conditions et
refuse `--sdp-authorized` en dehors du contrat. Ne jamais contourner ce refus en
repassant en confirmation manuelle : corriger l'état, ou traiter la gate comme
échouée.

## Bornage d'une revue Codex

Une revue Codex n'est bornée que par `codex.timeout_minutes` de la politique.
Le CLI Codex n'expose aucun plafond de tours. Ne pas promettre une limite en
tours dans un document canonique et ne pas traiter `turns_observed` comme une
garantie : c'est une observation.
