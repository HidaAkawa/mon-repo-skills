...
1. Lire [references/configuration.md](references/configuration.md), puis exécuter immédiatement `python3 <skill-dir>/scripts/claude_review.py discover-models` avant toute question de configuration. Demander ensuite de manière bloquante le modèle Claude à enregistrer. Utiliser le sélecteur natif de l'environnement avec les identifiants exacts renvoyés et toujours permettre une saisie libre. Si `source` vaut `embedded_catalog`, signaler brièvement que la découverte API n'était pas disponible et que le catalogue versionné est utilisé. Si le sélecteur ne peut pas présenter toute la liste, proposer d'abord `claude-sonnet-5` et `claude-opus-5` lorsqu'ils figurent dans le résultat, compléter avec les premiers modèles renvoyés, puis proposer « autres modèles disponibles » et la saisie libre. Recommander `claude-sonnet-5` lorsqu'il figure dans le résultat, mais ne jamais choisir un modèle sans réponse.
